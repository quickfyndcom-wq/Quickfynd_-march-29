...
import Imageslider from '../assets/signin/6.webp';
import axios from 'axios';
import { countryCodes } from '../assets/countryCodes';

const SignInModal = ({ open, onClose, defaultMode = 'login', bonusMessage = '', disableQuickSignIn = false }) => {
  ...
  const linkGuestOrders = async (user, token) => {
    try {
      const email = String(user?.email || '').trim();
      const phone = String(user?.phoneNumber || '').trim();
      if (!email && !phone) return;

      await axios.post('/api/user/link-guest-orders', {
        email,
        phone,
      }, {
        headers: {
      ...
      // Non-blocking: fallback linking also happens in orders API + layout linker.
    }
  };
  ...
  const handleAuthSuccess = useCallback(async (user, isNewUser) => {
    ...
    try {
      ...
      await linkGuestOrders(user, token);
      if (isNewUser) {
  ...
  const handleQuickSignIn = async () => {
    ...
    try {
      if (auth.currentUser) {
        ...
        await trackLoginLocation(token);
        await linkGuestOrders(auth.currentUser, token);
    ...
    try {
      if (isRegister) {
        ...
        try {
          ...
          await trackLoginLocation(token);
          await linkGuestOrders(userCredential.user, token);
          await axios.post('/api/wallet/bonus', {}, {
            headers: { Authorization: `Bearer ${token}` }
          });
        } catch (err) {
