...
async function buildOrderStatusHtml({ name, order = {}, heroTitle, heroSubtitle, heroGradient, statusLabel, extraHtml = '' }) {
  ...
  const trackOrderParams = new URLSearchParams();
  if (order?._id) trackOrderParams.set('orderId', String(order._id));
  if (order?.shortOrderNumber) trackOrderParams.set('orderNo', String(order.shortOrderNumber));
  trackOrderParams.set('autoTrack', '1');
  const trackOrderUrl = `${STORE_BASE_URL}/track-order?${trackOrderParams.toString()}`;
...
<div class="wrap">
  ...
  <div class="body">
    ...
    <div class="card">
      ...
      <center>
        <a class="track-btn" href="${trackOrderUrl}">🔍 Track My Order</a>
      </center>
    </div>
    <div class="card">
...
export async function sendOrderStatusEmail(order, status) {
  const { shippingAddress, trackingId, trackingUrl, courier } = order;
  const normalizeString = (value) => (typeof value === 'string' ? value.trim() : '');

  const resolveRecipient = async () => {
    let email = normalizeString(order?.guestEmail) || normalizeString(order?.email) || normalizeString(shippingAddress?.email);
    let name = normalizeString(order?.guestName) || normalizeString(order?.name) || normalizeString(shippingAddress?.name);

    if ((!email || !name) && order?.userId && typeof order.userId === 'object') {
      if (!email) email = normalizeString(order.userId.email);
      if (!name) name = normalizeString(order.userId.name);
    }

    if ((!email || !name) && order?.addressId) {
      try {
        const { default: Address } = await import('@/models/Address');
        const addressDoc = await Address.findById(order.addressId).select('email name').lean();
        if (!email) email = normalizeString(addressDoc?.email);
        if (!name) name = normalizeString(addressDoc?.name);
      } catch (addressError) {
        console.error('[email] Failed to resolve recipient from address:', addressError);
      }
    }

    if ((!email || !name) && typeof order?.userId === 'string' && order.userId.trim()) {
      try {
        const { default: User } = await import('@/models/User');
        const userDoc = await User.findById(order.userId).select('email name').lean();
        if (!email) email = normalizeString(userDoc?.email);
        if (!name) name = normalizeString(userDoc?.name);
      } catch (userError) {
        console.error('[email] Failed to resolve recipient from user:', userError);
      }
    }
  ...
  const { email, name } = await resolveRecipient();
  if (!email) {
    throw new Error(`No recipient email found for order ${order?._id || 'unknown'}`);
  }
  ...
  switch (normalizedStatus) {
    ...
    default:
        return sendOrderCustomStatusEmail({ email, name, order, status: normalizedStatus });
  }
...
export async function sendOrderConfirmationEmail(orderData) {
  ...
  const confirmationTrackParams = new URLSearchParams();
  if (orderId) confirmationTrackParams.set('orderId', String(orderId));
  if (shortOrderNumber) confirmationTrackParams.set('orderNo', String(shortOrderNumber));
  confirmationTrackParams.set('autoTrack', '1');
  const confirmationTrackUrl = `${STORE_BASE_URL}/track-order?${confirmationTrackParams.toString()}`;
  ...
  const html = `
    ...
    <head>
      <style>
        ...
        .button { display: inline-block; background: #1d4ed8; color: #ffffff !important; padding: 12px 30px; text-decoration: none; border-radius: 10px; margin: 12px 0 6px 0; font-weight: 700; font-size: 15px; letter-spacing: 0.3px; line-height: 1.2; }
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="card" style="margin-top: 12px; text-align:center; background: linear-gradient(135deg, #eff6ff 0%, #f5f3ff 100%); border: 1px solid #c7d2fe;">
            <p style="margin:0 0 10px 0; color:#334155; font-size:14px;">Want the latest shipment updates?</p>
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:0 auto; border-collapse:separate;">
              <tr>
                <td bgcolor="#1d4ed8" style="border-radius:10px; background:#1d4ed8;">
                  <a
                    href="${confirmationTrackUrl}"
                    class="button"
                    style="display:inline-block; background:#1d4ed8; color:#ffffff !important; text-decoration:none; font-weight:800; font-size:15px; line-height:1.2; letter-spacing:.3px; padding:13px 28px; border-radius:10px; border:1px solid #1e40af;"
                  ><span style="color:#ffffff !important;">Track Order</span></a>
                </td>
              </tr>
            </table>
          </div>
