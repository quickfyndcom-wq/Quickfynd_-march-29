...
function sendOrderWaitingForPickupEmail({ email, name, order }) {
  ...
  const html = `
    <div style="background:#fbbf24;padding:32px 0;text-align:center;color:#fff;">
      <img src="${EMAIL_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
    ...
    <div style="padding:32px;background:#f8f9fa;">
      ...
      <p style="margin-top:30px;">Order ID: <b>${getDisplayOrderNumber(order)}</b></p>
    </div>
  `;
...
export async function sendOrderCustomStatusEmail({ email, name, order, status }) {
  const subject = `Order Update (${status}) - ${getDisplayOrderNumber(order)}`;
  const html = `
    <div style="background:#2874f0;padding:32px 0;text-align:center;color:#fff;">
      <img src="${EMAIL_LOGO_URL}" style="max-width:200px;margin-bottom:16px;"/>
  ...
  const html = `
    ...
    <div style="padding:32px;background:#f8f9fa;">
      ...
      <div style="margin-top:20px;">
        <a href="${reviewLink}" style="display:inline-block;background:#f59e0b;color:#111827;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:700;">⭐ Rate Your Delivery</a>
      </div>
      <p style="margin-top:30px;">Order ID: <b>${getDisplayOrderNumber(order)}</b></p>
    </div>
  `;
      ...
      <tr>
        <td style="padding: 15px; border-bottom: 1px solid #e5e7eb;">
          <div style="display: flex; align-items: center; gap: 15px;">
            ${productImage ? `<img src="${productImage}" alt="${productName}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px;">` : ''}
            <div>
...
export async function sendOrderShippedEmail(orderData) {
  ...
  const subject = `Order Shipped - ${getDisplayOrderNumberFromParts(null, orderId)}`;
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>Order #${getDisplayOrderNumberFromParts(null, orderId)}</p>
        </div>
        <div class="content">
          <h2>Hi ${name || 'there'}! 👋</h2>
  ...
  const html = `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p style="margin-bottom: 0;">Need help? Contact us at support@quickfynd.com</p>
          <p>© ${new Date().getFullYear()} QuickFynd. All rights reserved.</p>
        </div>
      </div>
    </body>
