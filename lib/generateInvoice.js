...
export const generateInvoice = async (order) => {
    ...
    const BRAND_DARK = [20, 56, 73];
    const BRAND_LIGHT = [236, 244, 248];
    const MUTED_TEXT = [107, 114, 128];
    ...
    try {
        ...
        drawImageContain(doc, img, margin.left, 6, 34, 30, 'PNG');
    } catch (e) {
    ...
    doc.setFontSize(18);
    doc.text('INVOICE', pageWidth - margin.right, 12.5, { align: 'right' });
    
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    doc.text('Invoice #', pageWidth - margin.right, 18.5, { align: 'right' });
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(11.5);
    const orderIdShort = String(order?._id || order?.id || '').slice(0, 8).toUpperCase();
    doc.text(`#${orderIdShort}`, pageWidth - margin.right, 24.5, { align: 'right' });
...
export const generateAwb = async (order, opts = {}) => {
    ...
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(10);
    doc.text(String(order?.shortOrderNumber || (String(order?._id || '').slice(0, 8).toUpperCase())), boxX + 28, boxY + 8);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(10);
