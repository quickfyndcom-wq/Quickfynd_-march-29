import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";
import logoSrc from "../assets/logo/logo1.png";
import { getDisplayOrderNumber } from "@/lib/orderNumber";

const COMPANY_NAME = "QuickFynd";
const COMPANY_ADDRESS_LINE1 = "Kerala, India";
const COMPANY_ADDRESS_LINE2 = "";
const COMPANY_CONTACT = "Email: support@quickfynd.com";
const COMPANY_GST = "";
// Keep ASCII-only to avoid emoji boxes if custom font fails to load
const THANK_YOU_LINE2 = process.env.NEXT_PUBLIC_INVOICE_QUOTE2 || "We hope you love your purchase!";

// Font config for proper ₹ rendering and better Unicode support
const UNICODE_FONT_NAME = 'RobotoJPDF';
const UNICODE_FONT_REG_VFS = 'Roboto-Regular.ttf';
const UNICODE_FONT_BOLD_VFS = 'Roboto-Bold.ttf';
// Allow override via env; otherwise use widely available Roboto TTFs
const UNICODE_FONT_REG_URL = process.env.NEXT_PUBLIC_INVOICE_FONT_URL ||
    'https://cdnjs.cloudflare.com/ajax/libs/ink/3.1.10/fonts/Roboto/roboto-regular-webfont.ttf';
const UNICODE_FONT_BOLD_URL = process.env.NEXT_PUBLIC_INVOICE_FONT_BOLD_URL ||
    'https://cdnjs.cloudflare.com/ajax/libs/ink/3.1.10/fonts/Roboto/roboto-bold-webfont.ttf';
let unicodeFontLoaded = false;

const arrayBufferToBase64 = (buffer) => {
    let binary = '';
    const bytes = new Uint8Array(buffer);
    const len = bytes.byteLength;
    for (let i = 0; i < len; i++) binary += String.fromCharCode(bytes[i]);
    return btoa(binary);
};

async function ensureUnicodeFont(doc) {
    if (unicodeFontLoaded) {
        try { doc.setFont(UNICODE_FONT_NAME, 'normal'); } catch {}
        return;
    }
    try {
        const [regRes, boldRes] = await Promise.all([
            fetch(UNICODE_FONT_REG_URL),
            fetch(UNICODE_FONT_BOLD_URL)
        ]);
        const [regBuf, boldBuf] = await Promise.all([
            regRes.arrayBuffer(),
            boldRes.arrayBuffer()
        ]);
        const reg64 = arrayBufferToBase64(regBuf);
        const bold64 = arrayBufferToBase64(boldBuf);
        doc.addFileToVFS(UNICODE_FONT_REG_VFS, reg64);
        doc.addFileToVFS(UNICODE_FONT_BOLD_VFS, bold64);
        doc.addFont(UNICODE_FONT_REG_VFS, UNICODE_FONT_NAME, 'normal');
        doc.addFont(UNICODE_FONT_BOLD_VFS, UNICODE_FONT_NAME, 'bold');
        unicodeFontLoaded = true;
        doc.setFont(UNICODE_FONT_NAME, 'normal');
    } catch (e) {
        // If font fails to load, fallback to core font (₹ may not render)
    }
}

// helpers
const formatInr = (n) => `₹${Number(n || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
const loadImage = (src) => new Promise((resolve, reject) => {
    try {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => resolve(img);
        img.onerror = reject;
        img.src = src;
    } catch (e) { reject(e); }
});

const drawImageContain = (doc, img, x, y, boxW, boxH, format = 'PNG') => {
    const iw = Number(img?.naturalWidth || img?.width || 1);
    const ih = Number(img?.naturalHeight || img?.height || 1);
    const ratio = Math.min(boxW / iw, boxH / ih);
    const drawW = Math.max(1, iw * ratio);
    const drawH = Math.max(1, ih * ratio);
    const dx = x + (boxW - drawW) / 2;
    const dy = y + (boxH - drawH) / 2;
    doc.addImage(img, format, dx, dy, drawW, drawH);
};

export const generateInvoice = async (order) => {
    const doc = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait' });
    await ensureUnicodeFont(doc);

    // A4 layout helpers
    const pageWidth = doc.internal.pageSize.getWidth();
    const pageHeight = doc.internal.pageSize.getHeight();
    const margin = { left: 15, right: 15, top: 15, bottom: 15 };
    const contentWidth = pageWidth - margin.left - margin.right;
    let y = margin.top;

    // ===== ENHANCED HEADER SECTION =====
    const BRAND_PRIMARY = [17, 63, 88];
    const BRAND_ACCENT = [37, 99, 235];
    const BRAND_SURFACE = [244, 248, 252];
    const MUTED_TEXT = [100, 116, 139];

    doc.setFillColor(...BRAND_PRIMARY);
    doc.rect(0, 0, pageWidth, 36, 'F');
    doc.setFillColor(...BRAND_ACCENT);
    doc.rect(0, 36, pageWidth, 3, 'F');

    // Logo only (preserve aspect ratio) on dark background
    try {
        const logoUrl = typeof logoSrc === 'string' ? logoSrc : (logoSrc?.src || logoSrc?.default || '');
        const img = await loadImage(logoUrl);
        drawImageContain(doc, img, margin.left, 5, 36, 24, 'PNG');
    } catch (e) {
        // keep header clean if logo fails to load
    }

    // Invoice identity on right
    doc.setTextColor(255, 255, 255);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(16);
    doc.text('INVOICE', pageWidth - margin.right, 11.5, { align: 'right' });
    
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    doc.text('Invoice #', pageWidth - margin.right, 17, { align: 'right' });
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(11);
    const displayOrderNumber = getDisplayOrderNumber(order);
    doc.text(`#${displayOrderNumber}`, pageWidth - margin.right, 22.5, { align: 'right' });

    // Reset text color
    doc.setTextColor(0, 0, 0);
    y = 46;

    // ===== TWO COLUMN LAYOUT: LEFT (BILL TO) + RIGHT (INVOICE DETAILS) =====
    const leftColX = margin.left;
    const rightColX = margin.left + contentWidth / 2 + 8;
    const colWidth = contentWidth / 2 - 4;

    // Resolve address fields (shippingAddress is the correct DB field)
    const addr = order?.shippingAddress || order?.address || {};
    const phoneCode = addr?.phoneCode || '+91';
    const phone = addr?.phone || 'N/A';

    // LEFT COLUMN: Bill To / Shipping Address
    doc.setFillColor(...BRAND_SURFACE);
    doc.roundedRect(leftColX - 3, y - 4, colWidth + 6, 30, 1.8, 1.8, 'F');
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(10);
    doc.setTextColor(...BRAND_PRIMARY);
    doc.text('BILL TO / SHIPPING ADDRESS', leftColX, y + 2);

    doc.setFontSize(9);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.text(String(addr?.name || order?.guestName || 'Customer'), leftColX, y + 10);
    
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    doc.setTextColor(31, 41, 55);
    let billY = y + 16;
    if (addr?.street) { doc.text(String(addr.street), leftColX, billY); billY += 5; }
    const cityLine = [addr?.city, addr?.state, addr?.zip].filter(Boolean).join(', ');
    if (cityLine) { doc.text(cityLine, leftColX, billY); billY += 5; }
    if (addr?.country) { doc.text(String(addr.country), leftColX, billY); billY += 5; }
    doc.text(`Phone: ${phoneCode} ${phone}`, leftColX, billY);
    if (addr?.alternatePhone) { billY += 5; doc.text(`Alt: ${addr?.alternatePhoneCode || phoneCode} ${addr.alternatePhone}`, leftColX, billY); }

    // RIGHT COLUMN: Invoice Details
    doc.setFillColor(...BRAND_SURFACE);
    doc.roundedRect(rightColX - 3, y - 4, colWidth + 6, 30, 1.8, 1.8, 'F');
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(10);
    doc.setTextColor(...BRAND_PRIMARY);
    doc.text('INVOICE DETAILS', rightColX, y + 2);

    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    doc.setTextColor(31, 41, 55);
    const invoiceDate = new Date(order?.createdAt || Date.now());
    
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.text('Invoice Date:', rightColX, y + 10);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.text(invoiceDate.toLocaleDateString('en-IN'), rightColX + 32, y + 10);

    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.text('Order No:', rightColX, y + 16);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.text(displayOrderNumber, rightColX + 32, y + 16);

    const paid = (String(order?.paymentMethod || '').toUpperCase() === 'STRIPE') ? (order?.isPaid ?? true) : (order?.isPaid ?? false);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.text('Payment Status:', rightColX, y + 22);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setTextColor(paid ? 22 : 220, paid ? 163 : 20, paid ? 74 : 60);
    doc.text(paid ? 'PAID' : 'UNPAID', rightColX + 32, y + 22);
    doc.setTextColor(0, 0, 0);

    // Tracking info if available
    if (order?.trackingId) {
        y += 36;
        doc.setFont(UNICODE_FONT_NAME, 'bold');
        doc.setFontSize(9);
        doc.setFillColor(255, 247, 237);
        doc.roundedRect(margin.left - 2, y - 3, contentWidth + 4, 16, 1.6, 1.6, 'F');
        doc.setTextColor(180, 83, 9);
        doc.text('TRACKING INFORMATION', margin.left, y + 2);
        
        doc.setFontSize(8);
        doc.setFont(UNICODE_FONT_NAME, 'normal');
        doc.text(`Tracking ID: ${order.trackingId}`, margin.left, y + 8);
        if (order?.courier) doc.text(`Courier: ${order.courier}`, margin.left + 68, y + 8);
        doc.setTextColor(0, 0, 0);
    }

    y += 32;

    // ===== ITEMS TABLE =====
    const tableData = (order?.orderItems || []).map((item, i) => {
        // item.name is the direct field in OrderItemSchema; item.product.name is fallback
        let productName = item?.name || item?.product?.name || item?.productName || 'Product';
        // Add variant info if present
        if (item?.variantOptions && typeof item.variantOptions === 'object') {
            const variants = Object.entries(item.variantOptions)
                .filter(([k, v]) => v)
                .map(([k, v]) => `${k}: ${v}`)
                .join(', ');
            if (variants) productName += `\n(${variants})`;
        }
        return [
            String(i + 1),
            productName,
            String(item?.quantity ?? 0),
            formatInr(item?.price ?? 0),
            formatInr((item?.price ?? 0) * (item?.quantity ?? 0))
        ];
    });

    const tableStartY = y;
    autoTable(doc, {
        startY: tableStartY,
        head: [["#", "Product Name", "Qty", "Unit Price", "Amount"]],
        body: tableData,
        theme: 'grid',
        styles: { 
            fontSize: 8, 
            cellPadding: 4,
            font: UNICODE_FONT_NAME,
            lineColor: 200,
            lineWidth: 0.3
        },
        headStyles: { 
            fillColor: BRAND_PRIMARY,
            textColor: 255, 
            fontStyle: 'bold', 
            lineWidth: 0.3,
            lineColor: 100
        },
        bodyStyles: { 
            lineWidth: 0.1, 
            lineColor: 220,
            fillColor: [255, 255, 255]
        },
        alternateRowStyles: {
            fillColor: [248, 248, 248]
        },
        margin: { left: margin.left, right: margin.right },
        columnStyles: {
            0: { cellWidth: 12, halign: 'center' },
            1: { cellWidth: contentWidth - (12 + 16 + 25 + 30) },
            2: { cellWidth: 16, halign: 'center' },
            3: { cellWidth: 25, halign: 'right' },
            4: { cellWidth: 30, halign: 'right', fontStyle: 'bold' }
        }
    });

    const tableBottom = doc.lastAutoTable?.finalY || tableStartY;
    const subtotal = (order?.orderItems || []).reduce((sum, it) => sum + ((it?.price ?? 0) * (it?.quantity ?? 0)), 0);
    const shippingFee = Number(order?.shippingFee ?? order?.shipping ?? 0);
    let discount = 0;
    if (order?.isCouponUsed && order?.coupon) {
        discount = order.coupon.discountType === 'percentage'
            ? (Number(order.coupon.discount || 0) / 100) * subtotal
            : Number(order.coupon.discount || 0);
    }

    y = tableBottom + 8;

    // ===== SUMMARY BOX (RIGHT SIDE) =====
    const summaryX = margin.left + contentWidth - 90;
    const summaryWidth = 88;
    
    // Summary background
    doc.setFillColor(...BRAND_SURFACE);
    doc.roundedRect(summaryX, y - 5, summaryWidth, 45, 1.6, 1.6, 'F');
    
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(9);
    doc.text('ORDER SUMMARY', summaryX + 5, y);

    doc.setFontSize(8);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    y += 7;
    
    doc.text('Subtotal:', summaryX + 5, y);
    doc.text(formatInr(subtotal), summaryX + summaryWidth - 5, y, { align: 'right' });
    
    y += 6;
    doc.text('Shipping:', summaryX + 5, y);
    doc.text(formatInr(shippingFee), summaryX + summaryWidth - 5, y, { align: 'right' });
    
    if (discount > 0) {
        y += 6;
        doc.setTextColor(34, 197, 94);
        doc.text('Discount:', summaryX + 5, y);
        doc.text(`-${formatInr(discount)}`, summaryX + summaryWidth - 5, y, { align: 'right' });
        doc.setTextColor(0, 0, 0);
    }

    // Total line
    y += 8;
    doc.setDrawColor(...BRAND_PRIMARY);
    doc.line(summaryX + 2, y, summaryX + summaryWidth - 2, y);
    
    y += 4;
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(10);
    doc.setFillColor(...BRAND_PRIMARY);
    doc.rect(summaryX, y, summaryWidth, 8, 'F');
    doc.setTextColor(255, 255, 255);
    doc.text('TOTAL', summaryX + 5, y + 5.5);
    doc.text(formatInr(order?.total ?? (subtotal + shippingFee - discount)), summaryX + summaryWidth - 5, y + 5.5, { align: 'right' });
    doc.setTextColor(0, 0, 0);

    // ===== FOOTER =====
    y = pageHeight - margin.bottom - 8;
    doc.setDrawColor(200, 200, 200);
    doc.line(margin.left, y, pageWidth - margin.right, y);
    
    y += 3;
    doc.setFontSize(7);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setTextColor(...MUTED_TEXT);
    doc.text('Thank you for shopping with QuickFynd!', pageWidth / 2, y, { align: 'center' });
    doc.setFontSize(6);
    doc.text('For support, visit www.quickfynd.com or email support@quickfynd.com', pageWidth / 2, y + 4, { align: 'center' });
    doc.setTextColor(0, 0, 0);

    return doc;
};

export const downloadInvoice = async (order) => {
    const doc = await generateInvoice(order);
    const idShort = getDisplayOrderNumber(order);
    doc.save(`Invoice_${idShort}.pdf`);
};

export const printInvoice = async (order) => {
    const doc = await generateInvoice(order);
    doc.autoPrint();
    const url = doc.output('bloburl');
    window.open(url, '_blank');
};

// ===== AWB / Shipping Label generation =====
export const generateAwb = async (order, opts = {}) => {
    const doc = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait' });
    await ensureUnicodeFont(doc);
    const pageWidth = doc.internal.pageSize.getWidth();
    const margin = { left: 12, right: 12, top: 12 };
    let y = margin.top;

    // Header
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(16);
    doc.text('AWB / Shipping Label', margin.left, y + 4);

    // Logo on the right
    try {
        const logoUrl = typeof logoSrc === 'string' ? logoSrc : (logoSrc?.src || logoSrc?.default || '');
        const img = await loadImage(logoUrl);
        drawImageContain(doc, img, pageWidth - margin.right - 40, y, 40, 24, 'PNG');
    } catch (e) {}

    y += 12;

    // FROM (Seller) block
    doc.setFontSize(9);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.text('FROM (Seller):', margin.left, y + 6);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    const sellerLines = [COMPANY_NAME, COMPANY_ADDRESS_LINE1, COMPANY_ADDRESS_LINE2, COMPANY_CONTACT].filter(Boolean);
    sellerLines.forEach((ln, i) => doc.text(String(ln), margin.left, y + 12 + i * 4));

    // RETURN / RTO address block (right side)
    const rightX = pageWidth / 2 + 6;
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(9);
    doc.text('RETURN / RTO ADDRESS:', rightX, y + 6);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    const returnAddr = opts.returnAddress || {
        name: COMPANY_NAME,
        line1: COMPANY_ADDRESS_LINE1,
        line2: COMPANY_ADDRESS_LINE2,
        contact: COMPANY_CONTACT
    };
    const retLines = [returnAddr.name, returnAddr.line1, returnAddr.line2, returnAddr.contact].filter(Boolean);
    retLines.forEach((ln, i) => doc.text(String(ln), rightX, y + 12 + i * 4));

    y += 28;

    // TO (Recipient)
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(9);
    doc.text('TO (Recipient):', margin.left, y + 6);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(9);
    const addr = order?.shippingAddress || order?.address || {};
    const toLines = [addr?.name || order?.guestName || 'Customer', addr?.street, [addr?.city, addr?.state, addr?.zip].filter(Boolean).join(', '), addr?.country, `Phone: ${addr?.phone || ''}`].filter(Boolean);
    toLines.forEach((ln, i) => doc.text(String(ln), margin.left, y + 12 + i * 4));

    // Order & Tracking box on right
    const boxX = pageWidth - margin.right - 70;
    const boxY = y;
    doc.setDrawColor(0);
    doc.setLineWidth(0.4);
    doc.rect(boxX, boxY, 70, 30);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(10);
    doc.text('Order:', boxX + 4, boxY + 8);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(10);
    doc.text(getDisplayOrderNumber(order), boxX + 28, boxY + 8);
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(10);
    doc.text('AWB:', boxX + 4, boxY + 16);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(10);
    doc.text(String(order?.trackingId || order?.delhiveryAwb || '------'), boxX + 28, boxY + 16);

    // Display selected contract and payment type if provided via opts
    try {
        const contractId = opts.contractId || opts.contract?.id || '';
        const contractLabel = opts.contractLabel || opts.contract?.label || '';
        const paymentType = (opts.paymentType || (order?.paymentMethod === 'cod' || String(order?.paymentMethod).toLowerCase() === 'cod' ? 'COD' : 'Prepaid'));
        if (contractLabel || contractId) {
            doc.setFont(UNICODE_FONT_NAME, 'bold');
            doc.setFontSize(9);
            doc.text('Contract:', boxX + 4, boxY + 26);
            doc.setFont(UNICODE_FONT_NAME, 'normal');
            doc.setFontSize(9);
            const contractText = contractLabel ? `${contractLabel}${contractId ? ` (${contractId})` : ''}` : contractId;
            doc.text(String(contractText), boxX + 28, boxY + 26);
        }
        if (paymentType) {
            doc.setFont(UNICODE_FONT_NAME, 'bold');
            doc.setFontSize(9);
            doc.text('Payment:', boxX + 4, boxY + 30 + 6);
            doc.setFont(UNICODE_FONT_NAME, 'normal');
            doc.setFontSize(9);
            doc.text(String(paymentType).toUpperCase(), boxX + 28, boxY + 30 + 6);
        }
    } catch (e) {}

    y += 36;

    // Products summary table (compact)
    doc.setFont(UNICODE_FONT_NAME, 'bold');
    doc.setFontSize(9);
    doc.text('Items:', margin.left, y + 6);
    doc.setFont(UNICODE_FONT_NAME, 'normal');
    doc.setFontSize(8);
    const items = (order?.orderItems || []).map(it => `${it.quantity || 1} x ${(it.name || it.product?.name || it.productName || '').substring(0,40)}`);
    items.slice(0, 8).forEach((it, i) => doc.text(it, margin.left + 6, y + 12 + i * 4));
    if (items.length > 8) doc.text(`+${items.length - 8} more`, margin.left + 6, y + 12 + 8 * 4);

    // Barcode / placeholder area
    const barcodeY = y + 12;
    const barcodeX = pageWidth - margin.right - 100;
    try {
        // Simple visual barcode: draw alternating bars based on order id hash
        const idStr = String(order?.trackingId || order?._id || order?.id || '');
        for (let i = 0; i < 40; i++) {
            const on = (idStr.charCodeAt(i % idStr.length) + i) % 2 === 0;
            if (on) doc.setDrawColor(0);
            else doc.setDrawColor(220);
            const bx = barcodeX + i * 2.2;
            doc.rect(bx, barcodeY, 2, 24, 'F');
        }
        doc.setFontSize(8);
        doc.setFont(UNICODE_FONT_NAME, 'normal');
        doc.text(String(order?.trackingId || ''), barcodeX, barcodeY + 30);
    } catch (e) {}

    return doc;
};

export const downloadAwb = async (order, opts = {}) => {
    const doc = await generateAwb(order, opts);
    const idShort = String(order?.shortOrderNumber || order?.id || '').toString();
    doc.save(`AWB_${idShort}.pdf`);
};
