...
const renderAwbBillPage = (doc, awbDetails, pageNumber = 1, totalPages = 1) => {
    ...
    sy += 4.5;
    const sellerPhone = String(awbDetails.senderPhone || DEFAULT_SELLER_PHONE);
    if (sellerPhone) {
        doc.setFont('helvetica', 'bold');
        doc.text('Ph:', margin.left + 3, sy);
        doc.setFont('helvetica', 'normal');
        doc.text(sellerPhone, margin.left + 12, sy);
    }
...
const renderAwbThermalBillPage = (doc, awbDetails, pageNumber = 1, totalPages = 1, ox = 0, oy = 0, scale = 1, rotate = false) => {
    ...
    // Fixed anchor: always sits at the bottom of the label regardless of content above
    ...
    // ── SELLER ────────────────────────────────────────────────
    y += 1.8;
    const sellerBottomLimit = retY - 1.2;
    secLabel('SELLER / FROM', y); y += 8.2;
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9.2);
    doc.setTextColor(0, 0, 0);
    doc.text(senderName.substring(0, 36), M, y + 1.1);
    y += 5.2;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.4);
    doc.setTextColor(0, 0, 0);
    const sLines = FIXED_SELLER_ADDRESS_LINES;
    const sellerMetaLineHeight = 3.4;
    const sellerAddressLineHeight = 3.2;
    for (const line of sLines) {
        doc.text(line, M, y);
        y += sellerAddressLineHeight;
    }
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(gst, M + 12, y);
    y += sellerMetaLineHeight;
    doc.setFont('helvetica', 'bold');
    doc.text('Contract ID :', M, y);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    doc.text(sellerContractId.substring(0, 28), M + 22, y);
    y += sellerMetaLineHeight;
    doc.setFont('helvetica', 'bold');
    doc.text('Customer ID:', M, y);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    doc.text(sellerCustomerId.substring(0, 18), M + 22, y);
    y += sellerMetaLineHeight;
    const sellerPhone = safeStr(awbDetails.senderPhone) || DEFAULT_SELLER_PHONE;
    if (sellerPhone) {
        doc.setFont('helvetica', 'bold');
        doc.text('Ph:', M, y);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(0, 0, 0);
        doc.text(sellerPhone, M + 9, y);
        y += sellerMetaLineHeight;
    }
