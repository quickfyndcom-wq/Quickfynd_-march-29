...
function findNestedDateValue(input, keyMatchers) {
  const seen = new Set();
  const stack = [input];
  ...
  while (stack.length > 0) {
    ...
    for (const [key, value] of Object.entries(current)) {
      ...
      if (keyMatchers.some((matcher) => matcher(normalizedKey)) && isLikelyDateValue(value)) {
        return value;
      }
...
function extractExpectedDates(shipment, statusObj) {
  const expectedDeliveryDate =
    ...
    statusObj.ExpectedDeliveryDate ||
    statusObj.expected_delivery_date ||
    findNestedDateValue(shipment, [
      (key) => key.includes('expected') && key.includes('delivery'),
      (key) => key === 'edd',
      (key) => key.includes('promise') && key.includes('date')
    ]);

  const expectedReturnDate =
    shipment.ExpectedReturnDate ||
    shipment.expected_return_date ||
    shipment.ReturnEDD ||
    shipment.return_edd ||
    statusObj.ExpectedReturnDate ||
    statusObj.expected_return_date ||
    findNestedDateValue(shipment, [
      (key) => key.includes('expected') && key.includes('return'),
      (key) => key.includes('return') && key.includes('date')
    ]);

  return {
    expectedDeliveryDate: expectedDeliveryDate || expectedReturnDate || '',
...
export function normalizeDelhiveryShipment(payload, fallbackWaybill = '') {
  ...
  const { expectedDeliveryDate, expectedReturnDate } = extractExpectedDates(shipment, statusObj);
