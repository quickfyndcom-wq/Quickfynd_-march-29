...
export default function StoreOrders() {
    ...
    const getPaymentStatus = (order) => {
        ...
        const rawPaymentStatus = String(order.paymentStatus || '').toUpperCase();
    ...
    const getEditablePaymentStatus = (order) => {
        const normalized = String(order?.paymentStatus || '').toUpperCase().trim();
        if (['PAID', 'FAILED', 'PENDING', 'REFUNDED'].includes(normalized)) return normalized;
    ...
    const saveEditedOrderDetails = async () => {
        ...
        if (!resolvedName || !resolvedPhone) {
            toast.error('Please fill name and phone before saving.');
            return;
        }

        const payload = {
            paymentMethod: normalizedPaymentMethod,
            paymentStatus: getEditablePaymentStatus(selectedOrder),
            shippingAddress: {
                ...shippingAddress,
                name: resolvedName,
                email: resolvedEmail,
                            ...
                            <div className="bg-slate-50 rounded-xl p-5">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm mb-4">
                                    ...
                                    <div>
                                        ...
                                        {isEditingOrderDetails ? (
                                            <select
                                                value={getEditablePaymentStatus(selectedOrder)}
                                                onChange={(e) => {
                                                    const nextPaymentStatus = String(e.target.value || '').toUpperCase();
                                                    setSelectedOrder({
                                                        ...selectedOrder,
