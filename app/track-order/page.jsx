...
function TrackOrderPageInner() {
  ...
  const trackOrderByInput = async ({ phone = '', awb = '', forceCarrier = false } = {}) => {
    if (!String(phone).trim() && !String(awb).trim()) {
    ...
    try {
      ...
      if (String(phone).trim()) params.append('phone', String(phone).trim())
      if (String(awb).trim()) params.append('awb', String(awb).trim())
      if (forceCarrier) params.append('carrier', 'delhivery')
      ...
      if (!forceCarrier && String(awb).trim() && status === 404) {
        try {
          const retry = await axios.get(`/api/track-order?carrier=delhivery&awb=${encodeURIComponent(String(awb).trim())}`)
  ...
  useEffect(() => {
    ...
    const shouldAutoTrack = ['1', 'true', 'yes'].includes(String(searchParams.get('autoTrack') || '').toLowerCase());
    const awbFromQuery = orderNo || orderId;

    if (awbFromQuery) {
      setAwbNumber(awbFromQuery);
    }

    if (awbFromQuery && !autoTracked && (shouldAutoTrack || orderId || orderNo)) {
      setAutoTracked(true);

      (async () => {
        let resolvedAwb = awbFromQuery;

        // For older links carrying only Mongo orderId, resolve and prefer short order number.
        if (orderId && !orderNo) {
          try {
            const orderRes = await axios.get(`/api/orders?orderId=${encodeURIComponent(orderId)}`);
            const resolvedOrderNo = String(
              orderRes?.data?.order?.shortOrderNumber ||
              orderRes?.data?.orderNumber ||
              orderRes?.data?.order?.orderNumber ||
              ''
            ).trim();

            if (resolvedOrderNo) {
              resolvedAwb = resolvedOrderNo;
              setAwbNumber(resolvedOrderNo);
            }
          } catch (resolveError) {
            // Fallback to provided orderId if order number lookup fails.
            console.warn('Failed to resolve short order number from orderId:', resolveError);
          }
        }

        trackOrderByInput({ awb: resolvedAwb, forceCarrier: false });
      })();
    }
  ...
  const handleTrack = async (e) => {
    e.preventDefault()
    trackOrderByInput({ phone: phoneNumber, awb: awbNumber, forceCarrier: forceDelhivery })
  }
  ...
  const normalizeStatusForTimeline = (status) => {
    const normalized = String(status || '').trim().toUpperCase();
    if (!normalized) return 'ORDER_PLACED';

    const aliases = {
      RETURN_INITIATED: 'RETURN_REQUESTED',
      RETURN_IN_PROCESS: 'RETURN_REQUESTED',
      RETURN_PICKED_UP: 'RETURN_REQUESTED',
      RETURN_IN_TRANSIT: 'RETURN_REQUESTED',
  ...
  const preferredOrderNo = String(
    order?.shortOrderNumber || order?.orderNumber || awbNumber || searchParams.get('orderNo') || ''
  ).trim()
  const preferredOrderId = String(order?._id || searchParams.get('orderId') || '').trim()
  const returnQuery = new URLSearchParams()
  if (preferredOrderNo) returnQuery.set('orderNo', preferredOrderNo)
  if (preferredOrderId) returnQuery.set('orderId', preferredOrderId)
  if (preferredOrderNo || preferredOrderId) returnQuery.set('autoTrack', '1')
  const returnToTrackOrder = `/track-order${returnQuery.toString() ? `?${returnQuery.toString()}` : ''}`
  const loginHref = `/sign-in?redirect_to=${encodeURIComponent(returnToTrackOrder)}`
          ...
          {order && (
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-xl font-semibold text-slate-800">
                      {displayOrderNumber ? `Order No: ${displayOrderNumber}` : `Order ID: ${(order._id || order.id || '')}`}
                    </h2>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mt-6 text-center">
                <p className="text-blue-700 font-medium">For full order details and history, please <a href={loginHref} className="underline text-blue-800">login</a> to your account. You will also receive an email with order information after every update.</p>
              </div>
            </div>
          )}
        </div>
